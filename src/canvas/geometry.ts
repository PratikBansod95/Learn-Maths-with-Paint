import type { Point, Region } from '../game/types';
...
/** Font size scaled to fit inside region. */
export function labelFontSize(region: Region): number {
  const b = boundsOf(region.points);
  const base = Math.min(b.width, b.height) * 0.42;
